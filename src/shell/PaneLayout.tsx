...
export const PaneLayout: React.FC<PaneLayoutProps> = ({ left, center, right }) => {
  return (
    <div className="flex h-full">
      {left && (
        <aside className="hidden md:flex w-72 border-r border-border bg-slate-950/80">
          <div className="flex-1 overflow-y-auto p-4">{left}</div>
        </aside>
      )}
      <main className={cn("flex-1 flex flex-col", right ? "md:flex-row" : "")}>
        <section className={cn("flex-1 overflow-y-auto", right && "md:border-r border-border")}>
          {center}
        </section>
        {right && (
          <aside className="w-full md:w-80 border-l border-border bg-slate-950/70">
