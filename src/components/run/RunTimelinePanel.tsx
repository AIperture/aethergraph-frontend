...
export const RunTimelinePanel: React.FC<RunTimelinePanelProps> = ({ nodes }) => {
  ...
  const listSorted = React.useMemo(() => {
    return [...meaningfulNodes].sort((a, b) => {
      const getRank = (s: string) => {
        const simple = normalizeStatus(s);
        // Priority: running > waiting > pending > failed > succeeded/other
        if (simple === "running") return 0;
        if (simple === "waiting") return 1;
        if (simple === "pending") return 2;
        if (simple === "failed") return 3;
        return 4;
      };
  ...
  const hasActive = React.useMemo(
    () =>
      timelineEntries.some((e) => {
        const s = normalizeStatus(e.node.status);
          ...
          <div className="flex-1 overflow-y-auto relative">
            ...
            <div className="divide-y divide-border/30 relative z-10">
              {timelineEntries.map(({ node, start, end, duration }) => {
                ...
                const simpleStatus = normalizeStatus(node.status);

                let barColor = "bg-emerald-500"; // succeeded/default
                if (simpleStatus === "failed") barColor = "bg-red-500";
                if (simpleStatus === "running") barColor = "bg-brand animate-pulse";
                if (simpleStatus === "pending") barColor = "bg-amber-400";
                if (simpleStatus === "waiting") barColor = "bg-sky-400";
                if (simpleStatus === "canceled") barColor = "bg-slate-400";
